...
const BG = require("./Bible/BG.json");
...
client.on("messageCreate", async (message) => {
  if (message.author.bot) return;
    ...
    // Uwzględnienie polskich znaków i przerw między nazwami ksiąg
    const commandRegex = /\b(\d*\s*[\wąćęłńóśźż]+(?:\s*[\wąćęłńóśźż]+)*)\s+(\d+):(\d+(-\d+)?)\b/gi;
    const matches = message.content.matchAll(commandRegex);

    for (const match of matches) {
      const book = getBook(match[1]); // Uzyskanie kanonicznej nazwy księgi z obiektu NameBooks
